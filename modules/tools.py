...
def get_moves(board, row, col,mycolor, is_sorted = False):
...
def get_jumps(board, row, col,mycolor, is_sorted = False):
    down, up = [(+1, -1), (+1, +1)], [(-1, -1), (-1, +1)]
    length = board.get_length()
    piece = board.get(row, col)
    if piece:
...
def search_path(board, row, col, path, paths,mycolor, is_sorted = False):

    path.append((row, col))
    jumps = get_jumps(board, row, col,mycolor, is_sorted)
    if not jumps:
        paths.append(path)
    else:
